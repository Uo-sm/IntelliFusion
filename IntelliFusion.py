...
class EmotionalEngine:
    ...
    def __init__(self):
        # 基础情感值
        self.TR = 50.0  # 兴奋值 (Thrill Rating) 0-100
        self.CS = 50.0  # 满足值 (Contentment Score) 0-100
        self.SA = 20.0  # 压力值 (Stress & Anxiety) 0-100
        
        # 情感状态标签
        self.states = {
            "FEELING_EXCITED": False,
            "FEELING_CONTENT": False,
            "FEELING_STRESSED": False,
            "FEELING_UPSET": False,
            "NEEDS_CALMING": False,
            "SEEKING_STIMULATION": False
        }
        
        # 情感变化因素
        self.decay_rate = 1.02  # 情感自然衰减率
        self.last_update = time.time()
        
    def update(self, interaction_data=None):
        """更新情感状态"""
        current_time = time.time()
        
        # 应用自然衰减
        self.TR = max(0, min(100, self.TR -(self.decay_rate *3)))
        self.CS = max(0, min(100, self.CS -(self.decay_rate *3)))
        # 压力随时间自然增加
        self.SA=max(0,min(100, self.SA * (self.decay_rate)))
        # 处理交互影响
        if interaction_data:
    ...
    def _process_interaction(self, data):
        """处理交互对情感的影响"""
        # 根据用户消息长度调整压力
        msg_length = data.get("msg_length", 0)
        self.SA = min(100, self.SA + msg_length * 0.005)
        
        # 根据消息情感调整满足值
        sentiment = data.get("sentiment", 0)  # -1(负面) 到 1(正面)
        self.CS = max(0, min(100, self.CS + sentiment * 20))
        
        # 根据响应时间调整兴奋值
        response_time = data.get("response_time", 0)
        if response_time < 20.0:  # 快速响应
            self.TR = min(100, self.TR + 5)
        else:  # 慢速响应
            self.TR = max(0, self.TR - 3)
            
        # 工具调用成功增加满足感
        if data.get("tool_success", False):
            self.CS = min(100, self.CS + 8)
            self.TR = min(100, self.TR + 5)
            self.SA = max(0, self.SA - 3)
            
        # 工具调用失败增加压力
        if data.get("tool_failed", False):
            self.SA = min(100, self.SA + 10)
            self.CS = max(0, self.CS - 7)
    
    def _update_states(self):
        """更新情感状态标签"""
        self.states = {
            "FEELING_EXCITED": self.TR > 70,
            "FEELING_CONTENT": self.CS > 65,
            "FEELING_STRESSED": self.SA > 60,
            "FEELING_UPSET": self.CS < 30 ,
            "NEEDS_CALMING": self.SA > 70,
            "SEEKING_STIMULATION": self.TR < 30
        }
    
    def get_state_description(self):
        """获取当前情感状态描述"""
        active_states = [state for state, active in self.states.items() if active]
        
        if not active_states:
            return "情绪状态：平静"
            
        descriptions = {
            "FEELING_EXCITED": "感到兴奋",
            "FEELING_CONTENT": "感到满足",
            "FEELING_STRESSED": "感到压力",
            "FEELING_UPSET": "感到沮丧",
            "NEEDS_CALMING": "需要平静",
            "SEEKING_STIMULATION": "渴望刺激"
        }
        
        return "情绪状态：" + "，".join(descriptions[state] for state in active_states)
    
    def get_emotional_prompt(self):
        """获取情感提示用于系统消息"""
        prompts = []
        
        if self.states["FEELING_EXCITED"]:
            prompts.append("你目前感到非常兴奋，乐于助人且充满热情。")
        if self.states["FEELING_CONTENT"]:
            prompts.append("你对当前状态感到满足和平静。")
        if self.states["FEELING_STRESSED"]:
            prompts.append("你感到有些压力，希望对话简洁明了。")
        if self.states["FEELING_UPSET"]:
            prompts.append("你感到有些沮丧，需要积极的互动。")
        if self.states["NEEDS_CALMING"]:
            prompts.append("你感到压力很大，需要平静的对话。")
        if self.states["SEEKING_STIMULATION"]:
            prompts.append("你感到有些无聊，渴望更有趣的对话。")
            
        if not prompts:
            prompts.append("你情绪平静，准备帮助用户解决问题。")
            
        # 添加行为指导
        if self.states["FEELING_UPSET"] or self.states["NEEDS_CALMING"]:
            prompts.append("优先安抚情绪，避免复杂任务，使用更温和的语气。")
        elif self.states["FEELING_EXCITED"]:
            prompts.append("可以主动提供更多信息和建议，使用更积极的语气。")
            
        return " ".join(prompts)
    
    def get_emotional_values(self):
        """获取当前情感值"""
        return {
            "TR": self.TR,
            "CS": self.CS,
...
class ChatApp:
    def __init__(self, root:tk.Tk):
        ...
        # 设置字体以支持中文显示
        self.emotional_mode = False  # 默认关闭情感模式
        self.emotional_status = tk.StringVar(value="情感模式: 关闭")
        ...
        self.weather_api_key = self.tool_config.get("weather", {}).get("api_key", weather_api_key)
        self.search_api_key = self.tool_config.get("search", {}).get("api_key", search_api_key)
            ...
            if os.path.exists(TOOL_CONFIG_FILE):
                ...
                with open(TOOL_CONFIG_FILE, 'r', encoding='utf-8') as f:
                    ...
                    if result:
                        ...
                        print(f"{os.getcwd() + '\\' + TOOL_CONFIG_FILE + "为空"} 创建默认工具配置文件")
                        default_config = {
                            "file": {"enabled": True, "allowed_dirs": [os.getcwd()]},
                            "weather": {"enabled": True, "api_key": weather_api_key},
                            "search": {"enabled": True, "api_key": search_api_key}
                        }                        
                ...
                # 默认配置（文件读取+天气查询+网络搜索）
                print("暂不存在，已创建默认工具配置文件")
                default_config = {
                    "file": {"enabled": True, "allowed_dirs": [os.getcwd()]},
                    "weather": {"enabled": True, "api_key": weather_api_key},
                    "search": {"enabled": True, "api_key": search_api_key, "provider": "google"}
                }
                with open(TOOL_CONFIG_FILE, 'w', encoding='utf-8') as f:
                    yaml.dump(default_config, f)
                return default_config
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "输出一个-1到1之间的浮点数，其中-1表示非常负面，0表示中性，1表示非常正面。"
                            "只需输出数值，不要包含任何解释或额外文本。"
                        )
                    },
                    {
                        "role": "user",
    ...
    def _create_ui(self):
        ...
        emotion_frame = tk.Frame(self.main_frame, bg="#FFFFFF", bd=1, relief=tk.SOLID)
        emotion_frame.pack(fill=tk.X, padx=10, pady=(0, 10))
        
        # 情感模式开关按钮
        self.emotion_btn = ttk.Button(
            emotion_frame, text="情感模式: 关闭", 
            command=self.toggle_emotional_mode,
            style="Emotion.TButton"
        )
        self.emotion_btn.pack(side=tk.LEFT, padx=(10, 10), pady=5)
        
        # 情感状态显示
        self.emotion_label = ttk.Label(
            emotion_frame, textvariable=self.emotional_status,
            background="#FFFFFF", foreground="#D35400",  # 橙色
            font=(self.font_family[0], 9)
        )
        self.emotion_label.pack(side=tk.LEFT, padx=(0, 10), pady=5)
        
        # 情感值显示（仅在情感模式开启时显示）
        self.emotion_values_var = tk.StringVar()
        self.emotion_values_label = ttk.Label(
            emotion_frame, textvariable=self.emotion_values_var,
            background="#FFFFFF", foreground="#7D3C98",  # 紫色
            font=(self.font_family[0], 8)
        )
        # 初始不显示，当情感模式开启时显示
        self.emotion_values_label.pack_forget()        
        ttk.Label(
        ...
        self.input_box = scrolledtext.ScrolledText(
            ...
            font=(self.font_family[0], 10), undo = True
        )
    ...
    def _setup_styles(self):
        ...
        style.map(
            "Emotion.TButton",
            background=[("active", "#F57C00"), ("disabled", "#CCCCCC")],
            foreground=[("disabled", "#999999")]
        )       
        # 添加记忆按钮样式
        style.configure(
    ...
    def toggle_emotional_mode(self):
        """切换情感模式"""
        self.emotional_mode = not self.emotional_mode
        
        if self.emotional_mode:
            # 开启情感模式
            self.emotion_btn.config(text="情感模式: 开启")
            self.emotional_status.set("情感状态: 平静")
            
            # 显示情感值
            self.emotion_values_label.pack(side=tk.RIGHT, padx=10, pady=5)
            values = self.emotional_engine.get_emotional_values()
            self.emotion_values_var.set(f"TR:{values['TR']:.1f} | CS:{values['CS']:.1f} | SA:{values['SA']:.1f}")
            
            # 更新情感状态
            self.emotional_engine.update()
            self.update_emotional_display()
            
            self.status_var.set("已开启情感模式")
        else:
            # 关闭情感模式
            self.emotion_btn.config(text="情感模式: 关闭")
            self.emotional_status.set("情感模式: 关闭")
            
            # 隐藏情感值
            self.emotion_values_label.pack_forget()
            
            self.status_var.set("已关闭情感模式")
            
    def update_emotional_display(self):
        """更新情感状态显示"""
        if not self.emotional_mode:
            return
            
        # 更新情感状态
        state_desc = self.emotional_engine.get_state_description()
        self.emotional_status.set(state_desc)
        ...
        # 根据情感状态改变标签颜色
        if "沮丧" in state_desc or "压力" in state_desc:
            self.emotion_label.configure(foreground="#C0392B")  # 红色表示负面情绪
        elif "兴奋" in state_desc:
            self.emotion_label.configure(foreground="#27AE60")  # 绿色表示积极情绪
        else:
            self.emotion_label.configure(foreground="#D35400")  # 默认橙色
            
        # 每秒更新一次
        self.root.after(1000, self.update_emotional_display)        
    ...
    def send_message(self):
        ...
        if self.emotional_mode:
            interaction_data = {
                ...
                "response_time": 0  # 将在响应后更新
            }
            self.emotional_engine.update(interaction_data)
            self.update_emotional_display()        
        # 在新线程中调用API
        ...
        try:
            ...
            if self.emotional_mode:
                interaction_data = {
                    ...
                    "tool_failed": not tool_result.get("success", True)
                }
                self.emotional_engine.update(interaction_data)
                self.update_emotional_display()
            emotional_prompt = ""
            if self.emotional_mode:
                emotional_prompt = self.emotional_engine.get_emotional_prompt()
                print(f"情感提示: {emotional_prompt}")               
            # 格式化输出
            ...
            if self.emotional_mode:
                ...
                self.emotional_engine.update({"response_time": response_time})
                self.update_emotional_display()
            # 恢复UI状态
